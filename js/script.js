...
let finalDate = new Date("Aug 15, 2023 18:00:00");
...
let updateTime = () => {
    let now = new Date();
    let hours = now.getHours() % 12; //bcs the clock can only display in 12hours-format
    let minutes = now.getMinutes();
    let seconds = now.getSeconds();
    // console.log(now);
    clockH.style.transform = `rotate(${30*hours}deg)`;
    clockM.style.transform = `rotate(${6*minutes}deg)`;
    clockS.style.transform = `rotate(${6*seconds}deg)`;
}
